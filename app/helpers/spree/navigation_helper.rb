...
module Spree
  module NavigationHelper
    def spree_navigation_data
      Spree::Taxon.where(depth:0, hide_from_nav: false).map do |t|
        {title: t.name,url: t.permalink, items: t.children.map{|tax| {title: tax.name, url: tax.permalink}}}
      end
    rescue
      []
    end

    def spree_navigation_data_for_index
      Spree::Taxon.where(depth:0).map do |t|
        {title: t.name,url: t.permalink,items: t.children.map{|tax| {title: tax.name, url: tax.permalink}}}
      end
    rescue
      []
