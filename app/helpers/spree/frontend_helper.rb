module Spree
  module FrontendHelper
    ...
    def spree_breadcrumbs(taxon, _separator = '', product = nil, id)
      ...
      if taxon
        ...
        crumbs << ancestors.each_with_index.map do |ancestor, index|
          content_tag(:li, content_tag(
            ...
            ) << content_tag(:meta, nil, itemprop: 'position', content: index + 1), itemprop: 'url', href: seo_url(ancestor, params: permitted_product_params(id))
    ...
    def permitted_product_params(id)
      product_filters = available_option_types(id).map(&:name)
    ...
    def filtering_params(id)
      ...
      available_option_types(id).map(&:filter_param).concat(static_filters)
    end

    def filtering_params_cache_key(id)
      params.permit(filtering_params(id))&.reject { |_, v| v.blank? }&.to_s
    end

    def available_option_types_cache_key(id)
      @available_option_types_cache_key = Spree::OptionType.where(taxon_id: id).includes(:translations).includes(:option_values).to_a
    end
